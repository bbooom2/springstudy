...
public class UserServiceImpl implements UserService {
  ...
  @Override
	public void join(HttpServletRequest request, HttpServletResponse response) {
	  ...
	  } else if (location.isEmpty() && event.isEmpty()) {
		  agreecode = 1;
	 ...
	 try {
		 ...
		 if(joinResult == 1) {
			 ...
			 out.println("location.href='"+ request.getContextPath() +"/index.do';"); // 웰컴페이지로 이동 
		 } else {
			 out.println("alert('회원 가입에 실패했습니다.');");
			 out.println("history.go(-2);"); //두단계 전으로 이동. 약관페이지로 이동. 
	...
	public void login(HttpServletRequest request, HttpServletResponse response) {
	  ...
	  // ID, PW가 일치하는 회원이 있으면 로그인 실패
			 ...
			 try {
				 ...
				 out.println("<script>");
				 out.println("alert('회원 가입에 실패했습니다.');");
				 out.println("location.href='"+request.getContextPath()+"/user/index.do';"); 
				 out.println("</script>");
				 out.flush();
				 out.close();
			 }catch (Exception e) {
  ...
  @Override
	public void autoLogin(HttpServletRequest request, HttpServletResponse response) {
		...
		/*
		  	...
		  	1. 자동 로그인을 체크한 경우 (자동의 로그인은 본인의 아이디로 로그인 하는 게 아님. 자동로그인은 비밀번호의 입력이 없음. 중복이 없고, 다른 사람이 알기 어려운 정보를 이용해서 자동 로그인에서 사용할 ID를 결정한다.)
		  		1) session의 id를 DB의 AUTOLOGIN_ID에 저장한다. 
		  		2) 자동 로그인을 유지할 기간(예시 : 15일)을 AUTOLOGIN_EXPIRED_AT 칼럼에 저장한다.
		  		3) session의 id를 쿠키에 저장한다. (쿠키 : 각 사용자의 브라우저에 저장되는 정보) 
	  ...
	  if(chkAutoLogin != null) {
		  ...
		  userDTO.setAutologinExpiredAt(new Date(System.currentTimeMillis() + 1000 * 60 * 24 * 15 )); // 현재 + 15일 : java.sql.Date 클래스를 이용해서 작업을 한다. 
		  										// java.sql.Date 클래스는 타임스탬프를 이용해서 날짜를 생성한다. 
		  										// long 타입의 date는 타임스탬프밖에 없음. 
		  ...
		  cookie.setMaxAge(60 * 60 * 24 *15); // 초단위로 15일 지정 
		  cookie.setPath(request.getContextPath()); // 애플리케이션의 모든 URL에서 autologinId 쿠키를 확인한다. 
		  response.addCookie(cookie); 
		  //서버가 쿠키를 만들어서 서버(브라우저)에 보낸다. 서버가 브라우저로 보내는 걸 응답이라고 한다. 
	  }
  ...
  @Override
	public void logout(HttpServletRequest request, HttpServletResponse response) {
	  ...
	  Cookie cookie = new Cookie("autoLoginId", "");
	  cookie.setMaxAge(0); // 쿠키 유지 시간을 0초로 설정
	  cookie.setPath(request.getContextPath()); //  autologinId 쿠키의 path와 동일하게 설
	  response.addCookie(cookie); 
	  
	  // 2. session의 모든 정보를 지운다. 
	  ...
	  // 응답 
		 try {
			 ...
			 if(insertResult == 1 && deleteResult == 1) {
				 ...
				 out.println("history.go(-1);"); //한단계 전으로 이동. go대신 back으로 기재해도 동일한 결과 출력  
