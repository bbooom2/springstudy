...
@AllArgsConstructor // 생성자를 넣어서 오토와이어드 대체 
@Service
public class BbsServiceImpl implements BbsService {
	...
	public void loadBbsList(HttpServletRequest request, Model model) {
		...
		// 비긴과 엔드 값 구하기 위해서. 비긴과 엔드를 구해야 매퍼에서 던져줄 수 있음. 
	...
	@Transactional(readOnly = true) // INSERT, UPDATE, DELETE 중 2개 이상의 쿼리를 실행하는 경우 반드시 추가한다. DBConfig에 작업 해놨음. 신경 안 쓰고 추가하면 됨. 트랜잭션처리 허용하는 애너테이션 넣어놨고 매니저 빈도 만들어져있고. 그냥 진짜 만들면 됨! 
	@Override
	public int addReply(HttpServletRequest request) {
		...
		return addReplyResult; // 답글이 잘 달렸는지의 여부가 반환값. 
	}
