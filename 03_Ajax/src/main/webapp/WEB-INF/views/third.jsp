...
<script>
	function fn1() {
		$.ajax({
			...
			data: JSON.stringify({ // 문자열 형식의 JSON 데이터를 서버로 보낸다. 파라미터 이름이 없음에 주의한다. (서버에서 파라미터로 받을 수 없다) 지금까지는 'param=' + JSON.stringify 했지만 파라미터 이름이 없기 때문에 서버에서 받을 수 없음. 
				'name': $('#name').val(),
				'tel':$('#tel').val()
			}),
			...
			success: function(resData) {	// resData = {"name": "민경태", "tel": "010-1234-1234"}
				let str = '<ul>'; // 안닫아도 됨 자동으로 닫힘 
				str += '<li>' + resData.name; // 안닫아도 됨 자동으로 닫힘 
				str += '<li>' + resData.tel; // 안닫아도 됨 자동으로 닫힘 
				$('#result').html(str);
 			},
