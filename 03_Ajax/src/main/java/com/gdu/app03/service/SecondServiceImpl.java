...
public class SecondServiceImpl implements ISecondService {
	...
	/*
	 	ResponseEntity<T> <- 응답 타입을 기재하게 되어 있음 
	 	1. Ajax 응답 데이터를 생성하는 클래스
	 	2. 생성자 중 하나의 사용법 
	 		public ResponseEntity(@Nullable T body, @Nullable MultiValueMap<String, String> headers, HttpStatus status)
	 		1) @Nullable T body								   : 실제로 응답할 데이터 -> 응답데이터가 널이어도 오류가 발생하지 않게 되어있다. 
	 		2) @Nullable MultiValueMap<String, String> headers : 응답 헤더(대표적으로 Content-Type)
	 		3) HttpStatus status 							   : 응답 코드(200, 404, 500 등) -> 응답코드에 나 에러났어요 하고 보내주면 캐치가 없어도 에러쪽으로 넘어감 
	*/
	@Override
	public ResponseEntity<BmiVO> execute1(HttpServletRequest request) {
		...
		try { 
			...
			return new ResponseEntity<BmiVO>(new BmiVO(weight, height, bmi, obesity), HttpStatus.OK);  //  HttpStatus.OK : 응답코드 200과 같다 
		}catch(Exception e) {
	...
	public ResponseEntity<Map<String, Object>> execute2(BmiVO bmiVO) {
		...
		if(weight == 0 || height == 0) {
			return new ResponseEntity<Map<String,Object>>(HttpStatus.BAD_REQUEST); // 응답코드가 정상(200)이 아니므로 $.ajax의 error로 전달
		}
