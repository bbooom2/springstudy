...
/*
 	...
 	2. Bean 등록 방식 3가지 
 		1) root-context.xml에 <bean> 태그를 추가한 경우 
 			@ContextConfiguration(locations={"file:src/main/webapp/WEB-INF/spring/root-context.xml}) //중괄호 생략할수있지만 넣겟음, 참고할 파일의 경로를 넣을것. 루트컨텍스틍의 경로를 넣어주는 것 
 		
 		2) com.gdu.app04.config.AppConfig.java @Configuration + @Bean을 작성한 경우 
 			@ContextConfiguration(classes={classes={AppConfig.class}) //중괄호여는 이유는 참고해야할 경로가 2개일수있으니까 
...
@FixMethodOrder(MethodSorters.NAME_ASCENDING) // 메소드 이름의 오름차순 : 알파벳순으로 테스트를 수행한다. 
public class BoardUnitTest {
	
	// 단위테스트를하기 위해서는 DAO가 필요하다. 
	...
	@Test
	public void a1삽입테스트() { 
		BoardDTO board = new BoardDTO(0, "제목", "내용", "작성자", null, null); // 맨 앞에 0은 사용안한다는 뜻 
	...
	public void a2상세테스트() {
		...
		LOGGER.info(board.toString()); //  문자열로 바꿔서 전달, LOG 기능으로 대체 완료
	...
	public void a3수정테스트() {
		...
		BoardDTO board = new BoardDTO(1, "[수정]제목", "[수정]내용", null, null, null); // 작성자와 작성일은 수정 불가함 
	...
	public void a5목록테스트() {
		...
		//System.out.println("a5 - " + list); // LOG 기능으로 대체할 예정, 쓸거면 쓰되 반드시 지울것 
		LOGGER.info(list.toString()); // 문자열로 바꿔서 전달, LOG 기능으로 대체 완료
