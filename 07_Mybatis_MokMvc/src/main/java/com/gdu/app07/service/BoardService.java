...
public interface BoardService { // 목록서비스 소개하겠음. 서비스는 다오에게 값을 주고 다오에게 값을 받아옴 받아오는 것에 대한 이야기. 목록의 경우 다오로부터 어레이리스트를 받아오게 됨. 
	...
	public int addBoard(HttpServletRequest reuqest); // 컨트롤러로부터는 http서블릿 리퀘스트 받을 것 벗 보낼때는 Board로 보낼것. 다오는 받았으면 그대로 나가세요지만 서비스는 아님. 서비스는 로직 짜는 곳. 필요한 로직이 있으면 얼마든지 자바코드를 써서 만들 수 있음. 

	public int modifyBoard(HttpServletRequest reuqest); // addBoard와 동일한 방식 
	
	public int removeBoard(HttpServletRequest request); // 제이에스피로부터 파라미터가 넘어갔을텐에 컨트롤러는 파라미터를 http로 받겠다는 것 이거 그대로 서비스에게 주겠다. 가져가서 너가 꺼내라는 것. 그럼 꺼낸 정보를 인트값으로 바꿔서 주겠다.
	public void testTx();
}
