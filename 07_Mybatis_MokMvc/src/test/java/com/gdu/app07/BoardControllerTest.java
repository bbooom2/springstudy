...
// JUnit4
@RunWith(SpringJUnit4ClassRunner.class) //JUnit 사용하겠다. 


// ContextConfiguration -> Bean관리를 어떻게 하고 있는지  
// Spring Container에 Bean을 저장하는 방법 : root-context 이용, Config에 만들었던 @Configuration과 @Bean, 직접 클래스에 덧붙이는 애너테이션
// 테스트에서 사용할 Bean이 @Component로 생성되었기 때문에 component-scan이 작성된 servlet-context.xml의 경로를 작성한다.
@ContextConfiguration(locations = {"file:src/main/webapp/WEB-INF/spring/appServlet/servlet-context.xml"})
// 여러개가 작성되는 것처럼 (배열처럼) {} 안에 기재한다. 
...
public class BoardControllerTest {
	...
	/*
	 	...
	 	1. 가상 MVC 테스트이다. 
	 	2. Controller를 테스트할 수 있는 통합 테스트이다. // 컨트롤러 동작시키려면 매핑을 이용해야 구동 가능
	 	3. method(GET인지 POST인지) + mapping을 이용해서 테스트를 진행한다. 
	 */
	...
	//ServletContext + ApplicationContext (서블릿 기능이 내장되어 있음) 만들어져 있기 때문에 @Autowired를 해주면 사용 가능.
	@Autowired
	private WebApplicationContext webApplicationContext;
	
	// LOGGER 등록 (시스템 아웃이 아닌 로거로 결과를 찍는 방법) 
	...
	//JUnit 단위테스트에서는 setup 체크 후 생성하면 간단한 형식으로 @Before가 만들어진다.
	@Before
	...
	// 삽입결과는 FlashAttribute를 사용한다는 것을 기억. 
	@Test
	public void a1삽입테스트() throws Exception {
		...
		LOGGER.debug(mockMvc.perform(MockMvcRequestBuilders // request builder를 실행주는 메소드 
				...
				.param("title", "테스트제목") 			// 전달되는 파라미터 
				.param("content", "테스트내용")			// 전달되는 파라미터
				.param("writer", "테스트작성자"))		// 전달되는 파라미터 
					.andReturn() 						// 삽입결과 
					.getFlashMap() 						// 출력이 FlashAttribute일 때 getFlashMap을 사용한다. FlashAttribute로 저장된 결과 확인 
	...
	public void a2수정테스트() throws Exception {
		...
		LOGGER.debug(mockMvc.perform(MockMvcRequestBuilders // request builder를 실행주는 메소드 
				...
				.param("title", "테스트제목2") 			// 전달되는 파라미터 
				.param("content", "테스트내용2")		// 전달되는 파라미터 
				.param("boardNo", "1"))					// 전달되는 파라미터_파라미터는 늘 String을 요구하므로 큰따옴표 해야함. 
					.andReturn() 						// 수정결과 	
					.getFlashMap() 						// 출력이 FlashAttribute일 때 getFlashMap을 사용한다. FlashAttribute로 저장된 결과 확인 
						.toString()); 					// 최종 결과를 문자열로 바꿨다.
